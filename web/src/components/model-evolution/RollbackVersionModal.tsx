...
export function RollbackVersionModal({ family, version, onClose, onSuccess }: Props) {
  ...
  const handleRollback = async () => {
    try {
      ...
      toast.success(`Successfully rolled back to version ${version.version}.`);
      onSuccess();
    } catch {
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        <div className="p-6">
          ...
          <div className="mt-4 bg-amber-50 border border-amber-200 rounded-lg p-4 flex gap-3 text-amber-800">
            ...
            <p className="text-sm">
              Rollback updates the registry production marker and records a rollback event. 
              <strong> It does not change live endpoint routing in this phase.</strong>
            </p>
          </div>

          <div className="mt-6 flex flex-col gap-3 text-sm">
            <div className="flex justify-between py-2 border-b border-gray-100">
              <span className="text-gray-500">Rollback Target:</span>
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
          ...
          <Button variant="primary" onClick={() => void handleRollback()} disabled={loading}>
