...
export function MilestoneTracker({
  ...
  return (
    <ol
      className="flex min-w-150 w-full items-start justify-between"
      aria-label={t("runtime.progress")}
    >
      {milestones.map((milestone, index) => {
        ...
        return (
          <li
            ...
            aria-current={milestone.state === "active" ? "step" : undefined}
          >
            <div className="flex w-full items-center">
              <span
                className={`relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 transition-all ${milestoneTone[milestone.state]}`}
              >
                <Icon
                  className={
