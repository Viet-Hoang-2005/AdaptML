...
export function PromoteVersionModal({
  ...
  const handlePromote = async () => {
    try {
      ...
      toast.success(
        result.message ||
          `Version ${formatVersion(version.version)} promoted to ${alias}.`,
      );
      if (result.warning) {
      ...
      toast.error(
        getApiErrorMessage(
          error,
          "Failed to promote version. Please try again.",
        ),
      );
      setLoading(false);
    }
      ...
      <div className="bg-surface rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        <div className="p-6">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex gap-3 text-blue-800 shadow-sm">
            ...
            <p className="text-sm">
              Promote this version to a stable routing alias. Clients can call
              the alias endpoint without changing URLs when the target version
              changes.
            </p>
          </div>
