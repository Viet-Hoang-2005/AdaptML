    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm">
        <div className="bg-surface rounded-2xl shadow-xl w-full max-w-md overflow-hidden text-center p-8">
          ...
          <h3 className="text-xl font-bold text-foreground mb-2">Not Enough Versions</h3>
          <p className="text-sm text-muted-foreground mb-6">Add another version to compare model evolution.</p>
      ...
      <div className="bg-surface rounded-2xl shadow-xl w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden">
        <div className="flex justify-between items-center p-6 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-foreground">Compare Versions</h2>
              <p className="text-sm text-muted-foreground">Family: {family.display_name || family.name}</p>
            </div>
          </div>
          <button onClick={onClose} className="text-muted-foreground hover:text-muted-foreground transition-colors p-2 rounded-full hover:bg-muted">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-muted/50">
          <div className="grid md:grid-cols-[1fr_1fr_auto] gap-4 mb-6 rounded-xl border border-border bg-surface p-4 shadow-sm">
            <select
              className="w-full bg-surface border border-border rounded-lg px-3 py-2 text-sm font-semibold shadow-sm focus:ring-2 focus:ring-blue-500"
              value={leftId}
            ...
            <div className="space-y-6">
              <section className="rounded-xl border border-border bg-surface p-5 shadow-sm">
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">{comparison.recommendation.reason}</p>
                  </div>
              ...
              <section className="grid md:grid-cols-2 gap-4">
                {[comparison.left, comparison.right].map((version, index) => (
                  <div key={version.id} className="rounded-xl border border-border bg-surface p-5 shadow-sm">
                    <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{index === 0 ? 'Left' : 'Right'}</p>
                    <h3 className="mt-1 text-xl font-bold text-foreground">{formatVersion(version.version)}</h3>
                    <div className="mt-3 flex flex-wrap gap-2">
                      <span className="rounded-full bg-muted px-2 py-0.5 text-xs font-bold uppercase text-foreground">{version.stage}</span>
              ...
              <section className="rounded-xl border border-border bg-surface shadow-sm overflow-hidden">
                <div className="border-b border-border px-5 py-3">
                  <h3 className="text-sm font-bold uppercase tracking-wider text-foreground">Metrics Diff</h3>
                </div>
                  ...
                  <table className="min-w-full text-sm">
                    ...
                    <tbody className="divide-y divide-border">
                      {comparison.metrics_diff.length === 0 ? (
                        <tr><td colSpan={5} className="px-4 py-6 text-center text-muted-foreground">No metrics captured for these versions.</td></tr>
                      ) : comparison.metrics_diff.map(metric => (
                        <tr key={metric.name}>
                          <td className="px-4 py-2 font-semibold text-foreground">{metric.name}</td>
              ...
              <section className="grid xl:grid-cols-2 gap-6">
                <div className="rounded-xl border border-border bg-surface shadow-sm overflow-hidden">
                  <div className="border-b border-border px-5 py-3">
                    <h3 className="text-sm font-bold uppercase tracking-wider text-foreground">Params Diff</h3>
                  </div>
                  <div className="max-h-72 overflow-auto">
                    <table className="min-w-full text-sm">
                      <tbody className="divide-y divide-border">
                        {comparison.params_diff.length === 0 ? (
                          <tr><td className="px-4 py-6 text-center text-muted-foreground">No params captured.</td></tr>
                        ) : comparison.params_diff.map(param => (
                          <tr key={param.name} className={param.changed ? 'bg-amber-50/40' : ''}>
                            <td className="px-4 py-2 font-semibold text-foreground">{param.name}</td>
                            <td className="px-4 py-2 font-mono text-muted-foreground">{formatValue(param.left)}</td>
                            <td className="px-4 py-2 font-mono text-foreground">{formatValue(param.right)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div className="rounded-xl border border-border bg-surface p-5 shadow-sm">
                  <h3 className="text-sm font-bold uppercase tracking-wider text-foreground">Deployability / Deployment</h3>
                  <div className="mt-4 grid gap-3 text-sm">
                    ...
                    <p className="text-xs text-muted-foreground">{comparison.deployability_diff.left.reason || '-'}</p>
                    ...
                    <p className="text-xs text-muted-foreground">{comparison.deployability_diff.right.reason || '-'}</p>
              ...
              <section className="rounded-xl border border-border bg-surface p-5 shadow-sm">
                <h3 className="text-sm font-bold uppercase tracking-wider text-foreground">Artifacts / Weights Diff</h3>
                <div className="mt-4 grid md:grid-cols-4 gap-3 text-sm">
                  ...
                  <div className="rounded-lg bg-muted border border-border p-3"><strong>{comparison.artifact_diff.unchanged_count}</strong><br />Unchanged</div>
                </div>
