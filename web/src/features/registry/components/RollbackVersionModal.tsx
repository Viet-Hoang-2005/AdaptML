...
export function RollbackVersionModal({ family, version, onClose, onSuccess }: Props) {
  ...
  const handleRollback = async () => {
    try {
      ...
      toast.success(`Successfully rolled back to version ${formatVersion(version.version)}.`);
      onSuccess();
    } catch {
      ...
      <div className="bg-surface rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        <div className="p-6">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3 text-amber-800 shadow-sm">
            ...
            <p className="text-sm">
              Rollback updates the registry production marker and records a rollback event. <strong>It does not change live endpoint routing in this phase.</strong>
            </p>
          </div>
        ...
        <div className="p-4 bg-muted border-t border-border flex justify-end gap-3">
          ...
          <button 
            ...
            disabled={loading}
            className="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-lg transition-colors shadow-sm disabled:opacity-50"
          >
