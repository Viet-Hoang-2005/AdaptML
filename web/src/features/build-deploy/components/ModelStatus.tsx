...
function TrackerStep({ label, state }: { label: string; state: string }) {
  ...
  return (
    <div className="flex w-24 shrink-0 flex-col items-center gap-2">
      <div className={`flex h-6 w-6 items-center justify-center rounded-full bg-surface ring-4 ring-background ${colorClass}`}>
        {icon}
      </div>
...
function TrackerLine({ state }: { state: 'completed' | 'pending' }) {
  return (
    <div className="-mt-6 flex-1 shrink-0 px-2">
      <div className={`h-0.5 w-full rounded-full ${state === 'completed' ? 'bg-success' : 'bg-muted'}`} />
    </div>
  );
}
...
export function ModelStatus({ model }: { model: ModelProject }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-lg border border-warning/20 bg-warning-subtle p-6">
