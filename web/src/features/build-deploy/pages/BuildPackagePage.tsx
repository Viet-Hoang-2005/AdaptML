...
import type { ModelBuildFormValues, ModelFlavor } from '@/features/catalog/types';
...
import { buildModelProject, cancelBuild as cancelBuildProject, deployModelProject, triggerModelProjectBuild } from '@/features/build-deploy/api/buildDeployApi';
...
function DeployStep({
  ...
  const cancelBuild = async () => {
    if (modelId) {
      try {
        await cancelBuildProject(modelId);
      } catch (e) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <BuildSummaryItem label="Model" value={form.name || 'Untitled model'} />
        <BuildSummaryItem label="Version" value={form.version || 'v1'} />
        <BuildSummaryItem label="Access" value={`${form.access_mode} API`} />
        <BuildSummaryItem label="Flavor" value={form.flavor} />
        <BuildSummaryItem label="Artifact" value={form.source_artifact?.name || 'Missing artifact'} />
        <BuildSummaryItem label="Metrics" value={form.metrics_file?.name || 'Optional'} />
        <BuildSummaryItem label="Insights" value={form.model_insights_file?.name || form.feature_importance_file?.name || 'Optional'} />
      </div>
      ...
      <TerminalLogViewer
        ...
        modelId={modelId}
        buildId={buildId}
        getBuildPreview={async (id) => (await getModelProject(id)).package_preview_tree || []}
