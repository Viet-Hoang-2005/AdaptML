...
export default function ModelManagementPage() {
  ...
  const columns: ColumnDef<ModelProject>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => <Badge variant={"primary"}>{row.original.flavor || 'Not set'}</Badge>,
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        <div className="flex items-center gap-1">
          ...
          />
          <IconButton label="Delete model" icon={<Trash2 className="h-4 w-4 text-red-500" />} onClick={() => setModelToDelete(record)} />
        </div>
      );
      },
    },
  ...
  return (
    <div className="flex w-full flex-1 flex-col space-y-6">
      ...
      <PageContent>
        <div className="flex flex-1 flex-col px-6 py-6 space-y-4">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <Button
              size="md"
              onClick={() => navigate('/dashboard/management/model/upload/metadata')}
            >
          ...
          <div className="flex flex-1 flex-col">
            {isLoading ? (
              ...
              <Placeholder
                ...
                showModelName={false}
                action={!searchQuery && <Button size="md" onClick={() => navigate('/dashboard/management/model/upload/metadata')}>{t('upload')}</Button>}
              />
            ) : (
