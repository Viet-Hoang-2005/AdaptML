...
export default function BuildModelPage() {
  ...
  const buildActive = Boolean(build && ['pending', 'queued', 'building'].includes(build.status));
  ...
  useEffect(() => {
    if (stream.status && BUILD_TERMINAL_STATUSES.includes(stream.status as typeof BUILD_TERMINAL_STATUSES[number])) {
      void refreshBuild();
    }
  ...
  const terminalLogs = stream.error
    ...
    : stream.logs;
  const startDisabled = transitionState !== 'idle' || !buildForm.source_artifact;
