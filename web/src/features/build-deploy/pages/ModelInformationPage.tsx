...
export function ModelInformationPage({ modelId }: { modelId: string }) {
  ...
  useEffect(() => {
    getModelProject(modelId).then((result) => {
      setProject(result);
  ...
  if (!project || !form) return <p className="text-sm text-muted-foreground">Loading model metadata…</p>;

  const setField = <K extends keyof ProjectMetadataForm>(field: K, value: ProjectMetadataForm[K]) => {
  ...
  const save = async () => {
    ...
    try {
      ...
      setForm(toForm(result));
      setEditing(false);
      toast.success('Model metadata saved. The current image and registry version are unchanged.');
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">Project metadata is latest-only. Updating it does not rebuild the current image.</p>
        {editing ? (
          <div className="flex gap-2">
            <Button variant="secondary" size="md" onClick={() => { setForm(toForm(project)); setEditing(false); }}>Cancel</Button>
      ...
      {editing ? (
        <ProjectMetadataFields form={form} project={project} setField={setField} />
      ) : (
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <Info label="Reference data" value={project.reference_data?.name || '-'} />
        </div>
      )}
    </div>
  );
