...
import { deleteModelProject } from '@/features/catalog/api/catalogApi';
import { cancelBuild as cancelBuildProject, deployModelProject, triggerModelProjectBuild } from '@/features/build-deploy/api/buildDeployApi';
...
export default function MLflowZipPage({
  ...
  const cancelBuild = async () => {
    if (createdModelId) {
      try {
        await cancelBuildProject(createdModelId);
      } catch (e) {
      ...
      <div className="space-y-4 border-t border-gray-200 pt-6">
        ...
        {deploymentId && (
          <TerminalLogViewer
            ...
            onCompleted={() => { void queryClient.invalidateQueries({ queryKey: catalogQueryKeys.projects() }); }}
          />
        )}
      </div>
