      ...
      <div className="w-full max-w-lg rounded-xl border border-border bg-surface shadow-xl">
        ...
        <div className="space-y-6 px-6 py-6">
          {avatarPreview ? (
            ...
            <div className="space-y-4 rounded-lg border border-dashed border-border px-5 py-6 text-center">
              <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-muted text-muted-foreground">
                <ImageUp className="h-6 w-6" />
              </div>
              <div className="space-y-1 pb-1">
                <p className="text-lg font-semibold text-foreground">{t('avatarDialog.set')}</p>
                <p className="text-sm text-muted-foreground">{t('avatarDialog.setDescription')}</p>
              </div>
          ...
          {(historyLoading || avatarHistory.length > 0) && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-foreground">{t('avatarDialog.previous')}</h3>
                {historyLoading && <span className="text-xs font-semibold text-muted-foreground">{t('avatarDialog.loading')}</span>}
              </div>
              <div className="flex gap-3 overflow-x-auto pb-1">
                {avatarHistory.map((avatar) => (
                  <button
                    ...
                    aria-label={avatar.is_current ? t('avatarDialog.current') : t('avatarDialog.selectPrevious')}
                    title={avatar.is_current ? t('avatarDialog.current') : t('avatarDialog.useThis')}
                  >
