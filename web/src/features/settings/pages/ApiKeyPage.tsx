  ...
  const columns: ColumnDef<ModelProject>[] = [
    {
      ...
      cell: ({ row }) => (
        <input
          type="checkbox"
          aria-label={`Allow access to ${row.original.name}`}
    ...
    },
    {
      ...
      header: 'Description',
      cell: ({ row }) => <span className="text-muted-foreground">{row.original.description || 'No description'}</span>,
    },
  ];
          ...
          <div className="flex flex-col gap-2 pt-2">
            ...
            <div className="rounded-xl border border-gray-300 bg-white overflow-hidden mt-1">
              <DataTable
                ...
                pageSize={5}
                emptyMessage="No private models are available for scoped access."
              />
            </div>
          </div>
