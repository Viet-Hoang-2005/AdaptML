...
export default function Header({ onOpenNavigation }: { onOpenNavigation: () => void }) {
  ...
  return (
    <header className="relative z-30 flex h-16 shrink-0 items-center justify-between border-b border-border bg-surface px-3 md:px-4 xl:px-5">
      <button type="button" onClick={onOpenNavigation} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-surface text-muted-foreground hover:bg-muted hover:text-foreground md:hidden" aria-label={t('actions.openNavigation')}>
      ...
      <div className="mx-3 min-w-0 flex-1 sm:max-w-md lg:absolute lg:left-1/2 lg:top-1/2 lg:w-[min(420px,42vw)] lg:-translate-x-1/2 lg:-translate-y-1/2">
        <Popover.Root open={modelMenuOpen} onOpenChange={setModelMenuOpen}>
          <Popover.Trigger asChild>
            <button type="button" className="flex h-10 w-full items-center justify-between gap-3 rounded-surface border border-border bg-surface px-4 text-left text-sm font-semibold text-foreground shadow-sm transition-colors hover:border-input focus-visible:ring-2 focus-visible:ring-ring" aria-label={t('modelSelector.label')}>
          ...
          <Popover.Portal>
            <Popover.Content align="center" sideOffset={8} className="z-50 w-[min(92vw,26.25rem)] rounded-surface border border-border bg-surface p-2 shadow-(--shadow-overlay) animate-fade-in">
              <div className="relative mb-2">
                ...
                <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t('modelSelector.search')} className="h-10 w-full rounded-surface border border-input bg-surface pl-9 pr-9 text-sm text-foreground outline-none focus:border-primary" autoFocus />
                {search && <button type="button" onClick={() => setSearch('')} className="absolute right-2 top-1/2 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-compact text-muted-foreground hover:bg-muted" aria-label={t('modelSelector.clearSearch')}><X className="h-3.5 w-3.5" /></button>}
              </div>
              ...
              <div className="mt-2 border-t border-border pt-2">
                <button type="button" onClick={() => { setModelMenuOpen(false); navigate(routes.uploadModel); }} className="flex h-10 w-full items-center gap-2 rounded-surface px-3 text-sm font-semibold text-muted-foreground hover:bg-muted hover:text-foreground">
      ...
      <div className="flex min-w-0 shrink-0 items-center justify-end gap-2 sm:min-w-40 xl:min-w-48">
        ...
        <DropdownMenu.Root>
          <DropdownMenu.Trigger asChild>
            <button type="button" className="flex h-10 w-10 items-center justify-center rounded-surface border border-border bg-surface text-muted-foreground hover:text-foreground" aria-label={t('theme.current', { mode })} title={t('theme.current', { mode })}>
          ...
          <DropdownMenu.Portal>
            <DropdownMenu.Content align="end" sideOffset={8} className="z-50 min-w-44 rounded-surface border border-border bg-surface p-1.5 shadow-(--shadow-overlay) animate-fade-in">
              ...
              {themeOptions.map((option) => {
                ...
                return <DropdownMenu.Item key={option.value} onSelect={() => setMode(option.value)} className="flex h-9 items-center gap-2 rounded-compact px-2 text-sm text-foreground outline-none hover:bg-muted focus:bg-muted"><Icon className="h-4 w-4 text-muted-foreground" /><span className="flex-1">{t(`theme.${option.value}`)}</span>{mode === option.value && <Check className="h-4 w-4 text-primary" />}</DropdownMenu.Item>;
              })}
        ...
        <DropdownMenu.Root>
          ...
          <DropdownMenu.Portal>
            <DropdownMenu.Content align="end" sideOffset={8} className="z-50 w-56 rounded-surface border border-border bg-surface p-2 shadow-(--shadow-overlay) animate-fade-in">
              ...
              <DropdownMenu.Item onSelect={() => navigate('/dashboard/settings/profile')} className="mt-2 flex h-10 items-center gap-2 rounded-compact px-2 text-sm text-foreground outline-none hover:bg-muted focus:bg-muted"><Settings className="h-4 w-4 text-muted-foreground" /> {t('navigation.setting')}</DropdownMenu.Item>
              <DropdownMenu.Item onSelect={logout} className="flex h-10 items-center gap-2 rounded-compact px-2 text-sm text-danger outline-none hover:bg-danger-subtle focus:bg-danger-subtle"><LogOut className="h-4 w-4" /> {t('actions.logout')}</DropdownMenu.Item>
            </DropdownMenu.Content>
          </DropdownMenu.Portal>
        </DropdownMenu.Root>
      </div>
