...
export function TerminalLogViewer({
    ...
    const fetchLogs = async () => {
      ...
      try {
        ...
        if (succeeded) {
          ...
          const message = data.error || (data.status === 'cancelled' ? 'Process cancelled.' : 'Process failed.');
          setErrorMsg(message);
          toast.error(message);
          onCompleted?.(data.status);
        }
  ...
  }, [building, buildStatus, getBuildPreview, logKind, modelId, onBuildSuccess, onCompleted, resourceId, title]);
