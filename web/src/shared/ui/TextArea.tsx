...
export const TextArea = forwardRef<HTMLTextAreaElement, TextAreaProps>(function TextArea(
  { id, label, error, helperText, minHeight = 'min-h-24', className, onChange, ...props },
  ref,
) {
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      ...
      <textarea
        ...
        {...props}
      />
      {(error || helperText) && <span id={messageId} role={error ? 'alert' : undefined} className={cn('text-xs', error ? 'text-danger' : 'text-muted-foreground')}>{error || helperText}</span>}
    </div>
  );
});
