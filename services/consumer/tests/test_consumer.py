...
from src.batching import INFERENCE_EVENT_COLUMNS, PRODUCTION_DATA_COLUMNS
...
def test_build_production_data_excludes_failed_or_non_mapping_inputs():
    frame = main.build_production_data_dataframe(
        [
            record({
                "id": "accepted", "tenant_id": "tenant", "project_id": "project",
                "model_version_id": "version", "timestamp": "2026-01-01T00:00:00Z",
                "features": {"x": 1}, "prediction": "safe", "status_code": 200,
            }),
            record({
                "id": "failed", "model_version_id": "version", "features": {"x": 2},
                "status_code": 503,
            }),
            record({
                "id": "invalid", "model_version_id": "version", "features": [1, 2],
                "status_code": 200,
            }),
        ]
    )

    assert frame["id"].tolist() == ["accepted"]
    assert frame.loc[0, "inference_event_id"] == "accepted"
    assert frame.loc[0, "label_status"] == "unlabeled"
    assert frame.loc[0, "data_quality_status"] == "unchecked"
    assert not frame.loc[0, "training_eligibility"]


def test_build_automatic_drift_signals_is_deduplicated_and_replay_safe():
    signals = main.build_automatic_drift_signals(
        [
            record({"id": "a", "features": {}, "status_code": 200, "model_version_id": "version-b"}, offset=7),
            record({"id": "b", "features": {"x": 1}, "status_code": 200, "model_version_id": "version-a"}, offset=8),
            record({"id": "c", "features": {"x": 1}, "status_code": 200, "model_version_id": "version-a"}, offset=9),
            record({"id": "d", "features": {"x": 1}, "status_code": 200, "model_version_id": None}, offset=10),
        ]
    )
...
def test_flush_batch_commits_only_after_success(monkeypatch):
    ...
    monkeypatch.setattr(main, "save_inference_events_and_production_data_and_automatic_drift_signals", persist)
    ...
    assert list(persist.call_args.args[0]) == list(INFERENCE_EVENT_COLUMNS)
    assert list(persist.call_args.args[1]) == list(PRODUCTION_DATA_COLUMNS)
...
def test_flush_batch_rejects_mixed_partitions(monkeypatch):
    consumer = FakeConsumer([])
    monkeypatch.setattr(main, "save_inference_events_and_production_data_and_automatic_drift_signals", Mock(return_value=True))
...
def test_failed_partition_batch_is_retained_paused_and_retried(monkeypatch):
    ...
    retries = {}
    monkeypatch.setattr(main, "save_inference_events_and_production_data_and_automatic_drift_signals", Mock(side_effect=[False, True]))
