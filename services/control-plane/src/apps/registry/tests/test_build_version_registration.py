...
def test_each_successful_manual_build_allocates_next_version():
    ...
    first = register_successful_build(build=first, image_uri=f"build-{first.public_id}:latest", storage=storage)
    second = register_successful_build(build=second, image_uri=f"build-{second.public_id}:latest", storage=storage)
