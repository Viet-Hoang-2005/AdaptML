...
def filter_core_packages(req_file_path: Path, output_path: Path) -> bool:
    ...
    with req_file_path.open("r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            if pkg_name in CORE_PACKAGES:
                print(f"Filtering out core API/infrastructure package from dynamic requirements: {line_str}")
                continue
    ...
    if has_valid_reqs:
        ...
        print(f"Wrote {len(safe_lines)} safe requirements to {output_path}")
        return True
    return False
...
def main():
    ...
    if not model_uri:
        print("MODEL_URI is not set or empty. Skipping runtime init download.")
        return

    model_id = int(model_id_str) if model_id_str and model_id_str != "unknown" and model_id_str.isdigit() else 0

    print(f"Downloading/Extracting model artifact from {model_uri}...")
    try:
        source_dir = download_model_artifact(model_id, model_uri)
        print(f"Artifact extracted to {source_dir}")
