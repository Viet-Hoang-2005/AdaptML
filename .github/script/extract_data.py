...
def prep_nids_dynamic_data(file_path, output_dir):
    ...
    for label in label_mapping.values():
        ...
        print(f"[+] Nhãn {label:<10} | Có sẵn: {available_count:<8} -> Sẽ trích xuất: {target_count}")
    ...
    print(f"🔥 Đã lưu Kịch bản Drift - PortScan ({len(df_portscan)} dòng) tại: {out_portscan}")
    ...
    print(f"🔥 Đã lưu Kịch bản Drift - BruteForce ({len(df_bruteforce)} dòng) tại: {out_bruteforce}")
    ...
    print(f"🔥 Đã lưu Kịch bản Drift - WebAttacks ({len(df_webattacks)} dòng) tại: {out_webattacks}")
