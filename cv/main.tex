...
\resumeItem{Developed a Django REST as the central control system for managing user resources, model state, and an immutable registry, with Argo Workflows orchestrating all Kubernetes workloads (build, deploy, train, drift).}
...
\resumeItem{Implemented a shift-left CI gate on every PR: secret scanning (Gitleaks), Conventional Commit linting, path-filtered unit tests (JUnit/pytest/Vitest), Dockerfile linting (Hadolint) and security scanning (Checkov), dependency vulnerability scanning (Trivy filesystem), IaC validation (OpenTofu fmt/validate + Checkov), and static code analysis with quality gate enforcement (SonarCloud).}
\resumeItem{Built an automated CD pipeline that detects changed services, builds Docker images, scans for HIGH/CRITICAL vulnerabilities (Trivy), pushes to a private Harbor registry, then cryptographically signs each image and attests its CycloneDX SBOM using keyless Cosign (Sigstore OIDC), and finally opens a deploy PR that pins image digests in the Kustomization manifest for FluxCD to reconcile.}
