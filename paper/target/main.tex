...
\tikzset{
  component/.style={draw, rounded corners=2pt, align=center, font=\footnotesize,
    text width=4.55cm, minimum height=1.7cm, inner sep=4pt},
...
\begin{longtable}{@{}P{2.6cm}P{1.6cm}P{4.7cm}P{4.5cm}@{}}
\caption{Ánh xạ nhóm Data Curation.}\\
\toprule
\textbf{Component} & \textbf{Mức phủ} & \textbf{Bằng chứng trong repo} & \textbf{Khoảng trống} \\
\midrule
\endfirsthead
\toprule
\textbf{Component} & \textbf{Mức phủ} & \textbf{Bằng chứng trong repo} & \textbf{Khoảng trống} \\
...
\addlinespace
Model Repository & \statusyes & ModelVersion + ModelArtifact + ModelMetric trong registry app; immutable versions, metrics\_summary, params\_summary, artifacts (source, training\_output, package, mlflow, metrics, params, model\_insights, feature\_importance, input\_schema). & Rất tốt -- đây là điểm mạnh. \\
\addlinespace
...
\toprule
\textbf{Nhóm} & \textbf{Có} & \textbf{Một phần} & \textbf{Thiếu} & \textbf{Nhận định} \\
\midrule
...
Infrastructure \& Supporting Services (7) & 5 & 2 & 0 & Nền tảng tốt; thiếu cost/resource observability và interaction workflow hoàn chỉnh. \\
\midrule
\textbf{Tổng (39)} & \textbf{14} & \textbf{10} & \textbf{15} & Tính đủ 39 component; V2-only và optional components vẫn được giữ để minh bạch scope. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{tikzpicture}[node distance=5mm and 5mm]
  \node[component, fill=blue!7] (data) {
    \textbf{Data Curation}\\[-1pt]
    Data Collector · Data Source\\
    Data Preprocessor · Quality Checker\\
    Data Labeler
  };
  \node[component, fill=blue!7, right=of data] (storage) {
    \textbf{Storage \& Versioning}\\[-1pt]
    Dataset / Raw Data / Feature Store\\
    Code / Model / Artifact Repository\\
    ML Metadata · Feedback · Predictions
  };
  \node[component, fill=blue!7, right=of storage] (training) {
    \textbf{ML Training}\\[-1pt]
    Feature Engineering\\
    Experiment Pipeline · Continuous Training\\
    Model Evaluator · Fine-Tuning
  };

  \node[component, fill=green!8, below=of data] (cicd) {
    \textbf{CI/CD}\\[-1pt]
    ML Pipeline Builder / Deployer\\
    ML Model Deployer\\
    ML Component Builder / Deployer
  };
  \node[component, fill=green!8, below=of storage] (inference) {
    \textbf{Inference}\\[-1pt]
    Inference Service / Engine Pool\\
    Runtime Monitor · Trigger\\
    Comparison · Decision · XAI
  };
  \node[component, fill=green!8, below=of training] (infra) {
    \textbf{Infrastructure \& Supporting}\\[-1pt]
    Resource · Communication · Container\\
    Workflow · Log · API Gateway\\
    User Interaction Manager
  };
  \node[component, fill=orange!12, below=of inference] (decision) {
    \textbf{Decision loop đề xuất}\\[-1pt]
    Evidence → Diagnosis → Policy\\
    RetrainingRequest → EvaluationGate\\
    Shadow/Canary → Promote/Rollback
  };

  \draw[flowarrow] (data) -- node[above, font=\tiny]{raw / curated data} (storage);
  \draw[flowarrow] (storage) -- node[above, font=\tiny]{versioned inputs} (training);
  \draw[flowarrow] (training) -- node[left, font=\tiny]{package} (cicd);
  \draw[flowarrow] (cicd) -- node[above, font=\tiny]{deploy} (inference);
  \draw[flowarrow] (inference) -- node[right, font=\tiny]{production evidence} (decision);
  \draw[flowarrow, dashed] (decision.east) to[bend right=28]
    node[right, font=\tiny]{conditional CT} (training.south east);
  \draw[flowarrow, dashed] (inference.west) to[bend left=28]
    node[left, font=\tiny]{feedback} (data.south west);
  \draw[flowarrow, dotted] (infra.north) -- node[right, font=\tiny]{runtime support} (training.south);
  \draw[flowarrow, dotted] (infra.north west) to[bend right=18] (inference.south east);
\end{tikzpicture}
...
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[node distance=5mm and 5mm]
  \node[flowbox, fill=blue!7] (events) {Inference events\\+ delayed labels};
  \node[flowbox, fill=blue!7, right=of events] (evidence) {EvidenceWindow\\drift · quality · performance\\confidence · label coverage};
  \node[flowdecision, fill=orange!14, right=of evidence] (quality) {Quality\\valid?};
  \node[flowdecision, fill=orange!14, right=of quality] (labels) {Labels +\\sample size?};
  \node[flowbox, fill=orange!10, right=of labels] (diagnosis) {Diagnosis\\ranked causes\\+ evidence};
  \node[flowdecision, fill=orange!14, right=of diagnosis] (policy) {Policy\\action?};

  \node[flowbox, fill=red!9, below=of quality] (engineering) {Engineering incident\\no retrain};
  \node[flowbox, fill=yellow!18, below=of labels] (proxy) {Proxy warning\\increase sampling\\or hold};
  \node[flowbox, fill=gray!10, below left=of policy] (observe) {Observe\\no action};
  \node[flowbox, fill=red!9, below=of policy] (human) {Human review\\or investigation};
  \node[flowbox, fill=green!10, below right=of policy] (candidate) {Conditional Auto CT\\snapshot + train};

  \node[flowbox, fill=green!10, below=of candidate] (gate) {EvaluationGate\\offline metrics + quality};
  \node[flowbox, fill=green!10, right=of gate] (traffic) {Shadow → canary\\paired logs + guardrails};
  \node[flowbox, fill=green!10, right=of traffic] (outcome) {Promote\\or rollback};

  \draw[flowarrow] (events) -- (evidence);
  \draw[flowarrow] (evidence) -- (quality);
  \draw[flowarrow] (quality) -- node[above, font=\tiny]{yes} (labels);
  \draw[flowarrow] (quality) -- node[left, font=\tiny]{no} (engineering);
  \draw[flowarrow] (labels) -- node[above, font=\tiny]{yes} (diagnosis);
  \draw[flowarrow] (labels) -- node[left, font=\tiny]{no} (proxy);
  \draw[flowarrow] (diagnosis) -- (policy);
  \draw[flowarrow] (policy) -- node[above left, font=\tiny]{observe} (observe);
  \draw[flowarrow] (policy) -- node[right, font=\tiny]{human} (human);
  \draw[flowarrow] (policy) -- node[above right, font=\tiny]{auto} (candidate);
  \draw[flowarrow] (candidate) -- (gate);
  \draw[flowarrow] (gate) -- (traffic);
  \draw[flowarrow] (traffic) -- (outcome);
\end{tikzpicture}%
}
\caption{Workflow degradation-aware: quality gate và label sufficiency được kiểm tra trước diagnosis; diagnosis mới chọn observe, human review, engineering hoặc conditional Auto CT.}
...
\begin{quote}
\small
\textbf{Input:} EvidenceWindow $E$ với metrics $\{d_\text{drift}, q_\text{quality}, \Delta_\text{perf}, \Delta_\text{conf}, r_\text{labels}, n, CI_\text{perf}\}$\\[0.5em]
\textbf{Step 1} (Data Quality Gate):\\
  ...
  \hspace*{1em}\textbf{Action}: \texttt{ENGINEERING}, \textbf{Confidence}: high\\[0.3em]
...
\hspace*{1em}and the lower bound of $CI_\text{perf}$ confirms degradation,\\
  ...
  \hspace*{1em}\textbf{Confidence}: medium-high\\[0.3em]
...
\hspace*{1em}If $\Delta_\text{perf} < \theta_\text{perf}$ \textbf{and} $d_\text{drift} < \theta_\text{drift}$\\
  ...
  \hspace*{1em}\textbf{Action}: \texttt{HUMAN\_REVIEW}, \textbf{Confidence}: medium\\[0.3em]
...
\hspace*{1em}If $d_\text{drift} > \theta_\text{drift}$ \textbf{and} $|\Delta_\text{perf}| \approx 0$ \textbf{and} $r_\text{labels} \ge r_\text{min}$\\
  ...
  \hspace*{1em}\textbf{Confidence}: medium; otherwise \texttt{INSUFFICIENT\_EVIDENCE}\\[0.3em]
...
\hspace*{1em}If calibrated $\Delta_\text{conf}$ is significant \textbf{and} $r_\text{labels} < r_\text{min}$\\
  \hspace*{1em}$\Rightarrow$ \textbf{Diagnosis}: \texttt{CALIBRATION\_WARNING}, \textbf{Action}: \texttt{INCREASE\_SAMPLING}, \textbf{Confidence}: low\\[0.3em]
\textbf{Default}:\\
...
\begin{longtable}{@{}P{3.2cm}P{4.6cm}P{5.1cm}@{}}
...
\begin{tikzpicture}[node distance=6mm and 5mm]
  \node[flowbox, fill=blue!7] (candidate) {Candidate version\\immutable lineage};
  \node[flowbox, fill=green!10, right=of candidate] (gate) {Offline\\EvaluationGate};
  \node[flowbox, fill=yellow!18, right=of gate] (shadow) {Shadow\\duplicate request\\champion trả response};
  \node[flowbox, fill=yellow!18, right=of shadow] (canary) {Canary\\5 → 10 → 25 → 50\%};
  \node[flowbox, fill=orange!12, right=of canary] (guardrail) {Guardrails\\error · latency · risk};
  \node[flowdecision, fill=orange!14, right=of guardrail] (pass) {Gate\\pass?};
  \node[flowbox, fill=green!10, above right=of pass] (promote) {Promote 100\%\\alias switch};
  \node[flowbox, fill=red!9, below right=of pass] (rollback) {Rollback 0\%\\blue--green recovery};

  \draw[flowarrow] (candidate) -- (gate);
  \draw[flowarrow] (gate) -- (shadow);
  \draw[flowarrow] (shadow) -- (canary);
  \draw[flowarrow] (canary) -- (guardrail);
  \draw[flowarrow] (guardrail) -- (pass);
  \draw[flowarrow] (pass) -- node[above, font=\tiny]{pass} (promote);
  \draw[flowarrow] (pass) -- node[below, font=\tiny]{fail} (rollback);
\end{tikzpicture}%
}
\caption{Protocol champion--challenger đề xuất: candidate phải qua offline gate, shadow và guardrails trước khi promote; rollback là nhánh an toàn khi gate thất bại.}
...
\begin{longtable}{@{}P{1.4cm}P{2.8cm}P{4.0cm}P{5.2cm}@{}}
...
\begin{itemize}
  \item \textbf{model-server + consumer:} Hoàn chỉnh event contract để gửi prediction ID, confidence, latency và status. Consumer persist đầy đủ fields. Bảo đảm durable hand-off để feedback join được event.
  \item \textbf{control plane:} Thêm Feedback API, EvidenceWindow, DegradationEvent, Diagnosis, RetrainingRequest, EvaluationGate, TrafficExperiment entities. Giữ UUID, tenant scope, callback idempotency và PostgreSQL/S3 state ownership.
...
\begin{tabularx}{\linewidth}{@{}c P{1.8cm} P{3.2cm} Y@{}}
\toprule
...
S8 & Stretch & Runtime / infrastructure issue (confounder, không phải model drift) & Rollback/autoscale \\
\bottomrule
\end{tabularx}
\end{table}
...
Amou Najafabadi et al.~\cite{najafabadi2026} cung cấp systematic mapping study toàn diện nhất cho MLOps architecture. Google~\cite{google_mlops} đề xuất ba mức maturity MLOps (ML0--ML2) với CT ở ML2. Microsoft~\cite{microsoft_mlops} mô tả maturity model năm mức. Bayram và Ahmed~\cite{bayram2025} survey trustworthy ML in production với focus vào robustness.
