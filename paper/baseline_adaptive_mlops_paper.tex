...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\linewidth]{assets/training_flow.png}
\caption{Training pipeline: the control plane submits a Kubeflow \texttt{PyTorchJob} via Argo Workflows; the runner executes on Karpenter-provisioned EC2 nodes and uploads the resulting artifact to S3, with optional MLflow metric tracking.}
\label{fig:training-flow}
\end{figure}

\textit{Metadata and registry.} When a completed job is ingested, the control plane extracts metrics, parameters, insights, and deployability summaries, which are mirrored into Model Evolution upon registration. The Native Registry groups versions through family and version records linked to deployable \texttt{ModelAPI} records, with PostgreSQL as the per-tenant source of truth for ownership, lifecycle state, and drift results.

\textit{Packaging, serving, and monitoring.} The packager builds serving-compatible images via Argo/Docker, pushes them to Harbor, and creates a FastAPI deployment pod exposing a prediction endpoint. Prediction logs flow asynchronously through Redpanda to PostgreSQL, and Evidently computes drift reports on production windows; drift is an adaptation signal requiring human review and does not trigger automatic redeployment. Figure~\ref{fig:deploy-drift} illustrates both pipelines.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\linewidth]{assets/build_deploy_model_flow.png}
\caption*{(a) Upload, packaging, and deployment pipeline}
\vspace{6pt}
\includegraphics[width=0.8\linewidth]{assets/drift_flow.png}
\caption*{(b) Production logging and drift monitoring pipeline}
\caption{Prototype deployment pipelines: (a) model artifact upload triggers packaging via the Argo/Docker path, building and pushing a serving image to Harbor before creating a FastAPI deployment pod; (b) each prediction request emits a log to Redpanda, which a consumer persists to PostgreSQL; drift assessment is triggered automatically on threshold or manually by the user, with Evidently computing the report and the result stored for Model Evolution.}
\label{fig:deploy-drift}
\end{figure}

\textit{Deployment and operational scope.} The repository includes Docker Compose, Kubernetes/K3s manifests, and Argo workflow templates for prototype/testbed deployment; where Karpenter is configured, training pods scale on AWS-backed GPU/CPU node pools on demand. The implemented baseline covers training, registry, serving, production logging, drift reports, and a basic routing alias; autonomous promotion, canary deployment, and production-grade hardening remain future work.
