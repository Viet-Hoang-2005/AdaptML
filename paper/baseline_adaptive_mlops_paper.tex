...
\textit{Training execution.} Training jobs can be created from uploaded source packages or Source Editor code. In the Kubernetes-native path, the control plane validates source and reference data, submits a request to Argo Events, and Argo Workflows instantiate a Kubeflow \texttt{PyTorchJob} where the runner executes the configured entry point and uploads the resulting \texttt{model.tar.gz} to object storage. User scripts do not need to import MLflow; an optional \texttt{\_mlops} bundle may include training metrics, parameters, and an artifact manifest, but its absence does not invalidate the artifact.

\textit{Metadata and registry.} When a completed job is ingested, the control plane extracts metrics, parameters, insights, and deployability summaries, which are mirrored into Model Evolution upon registration. The Native Registry groups versions through family and version records linked to deployable \texttt{ModelAPI} records; Model Evolution displays tracking status, version comparison, deployment actions, drift summaries, and routing aliases, with PostgreSQL as the per-tenant source of truth.

\textit{Packaging, serving, and monitoring.} The packager prepares serving-compatible images through a local Docker or Argo workflow path. The FastAPI data plane loads artifacts, exposes generated prediction endpoints, and logs requests asynchronously through Redpanda/PostgreSQL for downstream drift analysis. Drift monitoring uses Evidently to compute feature-level and dataset-level summaries; drift is an adaptation signal requiring human review and does not trigger automatic redeployment. MLflow is optional internal tracking; if disabled, Model Evolution still uses Native Registry data in PostgreSQL.

\textit{Deployment and operational scope.} The repository includes Docker Compose, Kubernetes/K3s manifests, Argo workflow templates, and CI/CD configuration for prototype/testbed deployment. Where Karpenter is configured, training pods scale on AWS-backed GPU/CPU node pools, allowing on-demand compute provisioning without maintaining persistent training nodes. The implemented baseline covers authentication, upload, Kubernetes-native training, metadata ingestion, Model Evolution, version comparison, deploy and smoke-test actions, production logging, drift reports, and a basic routing alias. Autonomous retraining, automated promotion, canary or blue-green deployment, and production-grade hardening remain outside the current baseline.
