...
This section documents prototype boundaries, not production readiness or performance characteristics. The prototype uses a control-plane/data-plane split: the Django/DRF control plane stores tenant, job, registry, deployment, and drift metadata in PostgreSQL, while the FastAPI data plane resolves endpoints, loads artifacts, predicts, and emits log events through Redpanda/PostgreSQL for drift analysis.
...
The implementation also includes S3-compatible artifact storage, Kubeflow \texttt{PyTorchJob} workflows, a Python training runner, a model packager, optional MLflow tracking/model-format support, Evidently-based drift monitoring, and Native Registry/Model Evolution. For users, PostgreSQL-backed Native Registry data is the source of truth; MLflow is internal lineage support rather than a required interface.
...
\textit{Training execution.} Training jobs can be created from uploaded source packages or Source Editor code. In the Kubernetes-native path, the control plane validates source and reference data, prepares S3-compatible references and presigned URLs, and sends a request to Argo Events. Argo Workflows instantiate a Kubeflow \texttt{PyTorchJob}, where the runner executes the configured entry point and uploads the resulting \texttt{model.tar.gz} artifact to object storage.

The non-local path does not require user scripts to import MLflow or receive MLflow credentials. User scripts write model artifacts and optional metadata; the runner packages the output and a completion webhook updates the \texttt{TrainingJob}. The required output is \texttt{model.tar.gz}; an optional \texttt{\_mlops} bundle may contain training summary, metrics, parameters, model insights, artifact manifest, stdout/stderr logs, and warnings. Missing metadata reduces registry detail but does not invalidate the artifact.

\textit{Metadata and registry.} When a completed job is ingested, the control plane downloads the artifact, extracts metadata, and stores metrics, parameters, insights, manifest, and deployability summaries. These summaries are mirrored into Model Evolution when a model is registered from the job. The Native Registry groups versions through family and version records linked to deployable \texttt{ModelAPI} records, and Model Evolution displays tracking status, metrics, artifacts, deployability state, version comparison, deployment actions, drift summaries, and basic routing aliases.

The Native Registry is a product-level control-plane abstraction, not a replacement for existing MLOps platforms. Tenant ownership, access rules, lifecycle states, deployment aliases, drift summaries, and UI actions remain in PostgreSQL through tenant-scoped records, while Argo/Kubeflow, object storage, MLflow, and Evidently act as adapters. This lets imported and training-job artifacts enter one lifecycle with per-tenant ownership, version comparison, deployability review, drift-aware status, and deployment actions.

\textit{Packaging, serving, and monitoring.} The model packager prepares serving-compatible packages or images through a local Docker path or an Argo-oriented workflow path. The FastAPI model server loads artifacts, including MLflow pyfunc artifacts where available, exposes generated prediction endpoints, and logs prediction requests asynchronously for monitoring. Serving measurements are prototype observations rather than production service-level claims. Drift monitoring uses production records in PostgreSQL and model reference data; Evidently computes feature-level and dataset-level summaries, and the control plane stores report locations and drift metadata for Model Evolution.

In this baseline, drift is a monitoring and adaptation signal. It does not automatically redeploy a model, perform rollback, or select traffic weights; those decisions require evaluation gates and human review policies outside the current baseline. MLflow is optional internal tracking after artifact ingestion: if enabled, the control plane may log metrics, parameters, and metadata artifacts and store a run identifier; if disabled or unavailable, Model Evolution still uses Native Registry data in PostgreSQL.

\textit{Deployment and operational scope.} The repository contains Docker Compose, Kubernetes/K3s-oriented manifests, Argo workflow templates, CI/CD configuration, and deployment-configuration updates. These assets document prototype/testbed automation rather than production hardening.

Operationally, the frontend calls the Django/DRF control plane for upload, training, registry, deployment, and drift views. The control plane exchanges artifacts with S3-compatible storage, submits training requests to Argo Events, receives callbacks after Kubeflow \texttt{PyTorchJob} execution, ingests runner artifacts into the Native Registry, receives production logs through Redpanda and PostgreSQL, and stores drift summaries. Where Karpenter is configured, training pods can match AWS-backed training node pools, but workflow logs, pod status, artifact uploads, registry ingest records, and Karpenter events should be attached separately before stronger operational claims.
...
We also ran a bounded prototype serving replay against a testbed endpoint using Locust traffic and Prometheus metrics scraped from the serving pod. The 100-user run issued 4,343 client-side prediction requests over approximately 89 seconds before the replay dataset was exhausted. In this bounded replay, Locust observed an average request rate of 47.6 requests/s, an average response time of 1.74 s, and an observed p95 response time of 2.30 s. The run recorded four HTTP 502 responses. Prometheus reported a peak observed server-side rate of 144.1 requests/s and non-zero-window average handler latency of 1.60 s for the \texttt{/models/\{model\_id\_str\}/predict} handler. These observations suggest that the prototype completed most replay requests but began showing gateway errors and possible queueing or saturation symptoms in the model-serving path. No SLA, scalability, or production performance claim is made from this replay. Figure~\ref{fig:endpoint-benchmark} shows the Locust request-rate and latency time series used for this observation.

\begin{figure}[t]
\centering
\includegraphics[width=0.9\linewidth]{assets/chart-locust-benchmark.png}
\caption{Bounded prototype serving replay with 100 Locust users over an approximately 89-second window. Request rate and latency are observed from Locust's client-side history during the replay.}
