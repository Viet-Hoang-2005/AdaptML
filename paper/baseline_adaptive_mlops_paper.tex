...
Training execution & Argo + Kubeflow PyTorchJob + runner & Executes configured entry points and returns artifacts. \\
...
Training jobs can be created from uploaded source packages or from code stored through the Source Editor workflow. In the Kubernetes-native path, the control plane validates the selected model source and reference data, prepares S3-compatible source and data references, creates presigned input and output URLs, and sends a training request to Argo Events. Argo Workflows then instantiates a Kubeflow \texttt{PyTorchJob} resource in the configured training namespace. The training container executes the configured entry point, writes model files and optional metadata to the expected output directories, and uploads the resulting \texttt{model.tar.gz} artifact to object storage.

The non-local training path does not require the user script to import MLflow or receive MLflow credentials. Instead, the user script writes normal model artifacts and optional metadata files, and the runner is responsible for packaging the output in a format the control plane can ingest. After the workflow completes, a webhook updates the \texttt{TrainingJob} record with artifact locations; the control plane can then ingest tracking metadata and synchronize Native Registry and \texttt{ModelVersion} records when the trained artifact is registered as a model.
...
The AWS-facing boundary supports object storage and Kubernetes-native training execution where configured. Conceptually, the web frontend calls the Django/DRF control plane for model upload, training, registry, deployment, and drift views; the control plane exchanges artifacts with object storage and submits training requests to Argo Events; Argo Workflows creates Kubeflow \texttt{PyTorchJob} resources for training pods; the runner returns artifacts for Native Registry ingestion; the FastAPI server handles inference and emits logs to Redpanda; the consumer stores logs in PostgreSQL; and the drift service writes summaries back to the control plane. When the cluster is configured with Karpenter, pending training pods can match training-specific node pools and AWS EC2 node classes, and empty training nodes can be consolidated according to node-pool policy. Runtime evidence such as Kubeflow/Argo run logs, pod status, artifact uploads, registry ingest records, Karpenter NodePool or NodeClaim events, health checks, and screenshots should be attached separately before stronger operational claims.

The implemented baseline covers authentication, manual model upload, Kubernetes-native training submission, metadata ingestion, Model Evolution views, version comparison, build/deploy/smoke-test actions, production logging, drift report generation, and a basic routing alias. Its limitations are intentionally explicit: autonomous retraining, automated promotion, canary or blue-green deployment, weighted traffic routing, resource quota enforcement, billing, and production-grade hardening are outside the completed baseline. K3s/Kubernetes deployment, Kubeflow training runs, Karpenter provisioning behavior, and CI/CD status should be reported with separate runtime evidence.

\section{Evaluation Design}

Because this paper is a baseline, evaluation focuses on feasibility and system behavior rather than NIDS accuracy. The validation plan covers five aspects: model onboarding, inference serving, production logging, drift detection, and operational workflow reachability. Onboarding checks whether model packages or completed training artifacts can be validated, stored, registered, and assigned endpoint metadata. Serving and logging checks whether tabular requests produce predictions and queryable production logs. Drift validation compares reference and production windows and reports drifted features, drift ratio, dataset-level decision, and report artifacts. Operational validation should use Kubeflow/Argo run status, training pod logs, artifact storage paths, registry ingest results, container image references, Kubernetes status, health checks, Karpenter node-provisioning evidence where configured, and end-to-end training or inference traces when runtime evidence is available.
