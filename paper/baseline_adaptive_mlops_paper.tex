...
We also ran a serving benchmark against a deployed endpoint using Locust replay traffic and Prometheus metrics scraped from the serving pod. The 100-user run issued 4,343 client-side prediction requests over approximately 89 seconds before the replay dataset was exhausted. Locust measured an average throughput of 47.6 requests/s, an average response time of 1.74 s, and p95 response time of 2.30 s. The run recorded four HTTP 502 responses. Prometheus reported peak server-side throughput of 144.1 requests/s and non-zero-window average handler latency of 1.60 s for the \texttt{/models/\{model\_id\_str\}/predict} handler. These measurements suggest that at 100 concurrent users the serving path remains mostly available but is latency-bound and begins to show gateway errors, indicating queueing or saturation in the model-serving path. Figure~\ref{fig:endpoint-benchmark} shows the Locust throughput and latency time series used for this observation.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{assets/chart-locust-benchmark.png}
\caption{Prototype serving benchmark with 100 Locust users over an approximately 89-second replay window. Throughput and latency are measured from Locust's client-side history during the benchmark.}
